...
<? if ($arItems): ?>
    ...
    foreach ($arItems as $i => $arItem):?>
        ...
        <div class="services-list__wrapper <?= $itemWrapperClasses ?>">
            <div
                class="services-list__item <?= $itemClasses ?> <?= ($bDetailLink ? '' : 'services-list__item--cursor-initial') ?>"
                id="<?= $this->GetEditAreaId($arItem['ID']) ?>">
                <? if ($bShowImage && $imageSrc):?>
                    <div class="services-list__item-image-wrapper <?= $imageWrapperClasses ?>">
                        <? if ($bDetailLink): ?>
                        <a class="services-list__item-link <?= ($bImgSticky ? 'sticky-block' : '') ?>"
                           href="<?= $detailUrl ?>">
                            <? else: ?>
                            <span class="services-list__item-link <?= ($bImgSticky ? 'sticky-block' : '') ?>">
                                ...
                                <? else:?>
                                    <span class="services-list__item-image rounded-4 <?= $imageClasses ?>"
                    ...
                    ?>
                        ...
                        <? if (
                            ...
                            <? if ($bDetailLink):?>
                                <!--										<a class="arrow-all stroke-theme-target" href="--><?php //=$detailUrl
                                ?><!--">-->
                                <!--											--><?php //=CAllcorp3::showIconSvg(' arrow-all__item-arrow', SITE_TEMPLATE_PATH.'/images/svg/Arrow_map.svg');
                                ?>
                ...
                <? if ($bFonImg):?>
                    <? if ($bDetailLink):?>
                        <a class="services-list__item-link services-list__item-link--absolute"
                           href="<?= $detailUrl ?>"></a>
                    <?endif; ?>
                    ...
                    <div class="services-list__item-additional-text-wrapper">
                        <div class="services-list__item-additional-text-top-part">
                            <? if ($arItem['SECTIONS'] && $arParams['SHOW_SECTION'] != 'N'):?>
                                <div
                                    class="services-list__item-section font_13 color_light--opacity"><?= implode(', ', $arItem['SECTIONS']) ?></div>
                            <?endif; ?>

                            <div
                                class="services-list__item-title switcher-title font_<?= $arParams['NAME_SIZE'] ?> color_light">
                ...
                <div
                    class="services-list__item-text-wrapper flexbox <?= ($bShowBottom ? 'services-list__item-text-wrapper--has-bottom-part' . (($arParams['IMAGE_POSITION'] === 'TOP' || $bTRLImg) ? ' flexbox--justify-beetwen' : '') : '') ?>">
                    <? if ($bFonImg):?>
                        <? if ($bDetailLink):?>
                            <a class="services-list__item-link services-list__item-link--absolute"
                               href="<?= $detailUrl ?>"></a>
                    ...
                    <div
                        class="flexbox <?= ($bShowImage && $imageSrc ? 'maxwidth-theme--half flex-1' : 'maxwidth-theme') ?>"><?endif;
                        ?>

                        <div
                            class="services-list__item-text-top-part <?= ($bFonImg ? 'srollbar-custom scroll-deferred' : '') ?> <?= ($bSRLImg && !$arParams['NARROW'] && !($bShowImage && $imageSrc)) ? 'flex-1' : '' ?>">
                            <? if ($bItemsTypeElements && $arItem['SECTIONS'] && $arParams['SHOW_SECTION'] != 'N'):?>
                                <div
                                    class="services-list__item-section font_13 <?= ($bFonImg ? 'color_light--opacity' : 'color_999') ?>"><?= implode(', ', $arItem['SECTIONS']) ?></div>
                            <?endif; ?>

                            <div class="services-list-custom__item-title  font_<?//=$arParams['NAME_SIZE']
                            ?>">
                                <? if ($bDetailLink):?>
                                    <a class="dark_link color-theme-target"
                                       href="<?= $detailUrl ?>"><?= $arItem['NAME'] ?></a>
                                <? else:?>
                                ...
                                <? elseif ($arParams['IMAGE_POSITION'] === 'TOP_LEFT'):?>
                                    <? if ($bDetailLink):?>
                                        <? if ($arParams['ELEMENTS_ROW'] == 1):?>
                                            <a class="arrow-all arrow-all--wide stroke-theme-target"
                                               href="<?= $detailUrl ?>">
                            ...
                            <? if (
                                ...
                                <div class="services-list__item-preview-wrapper">
                                    <div
                                        class="services-list__item-preview font_15 <?= ($bFonImg ? 'color_light--opacity' : 'color_666') ?>">
                                        <?= $previewText ?>
                                    </div>
                                </div>
...
<!--                            --><?//endif; ?>
                                    ...
                                    <ul class="list-unstyled">
                                        ...
                                        <? foreach ($arItem['CHILD'] as $arChild):?>
                                            <li class="services-list__item-childs-item-wraper">
                                                ...
                                                <? if ($bShowChildDetail): ?>
                                                <a class="services-list__item-childs-item <?= ($bFonImg ? 'dark_link color_light--opacity' : '') ?>"
                                                   href="<?= ($arChild['DETAIL_PAGE_URL'] ? $arChild['DETAIL_PAGE_URL'] : $arChild['SECTION_PAGE_URL']) ?>">
                                                    <? else: ?>
                                                    <span
                                                        class="services-list__item-childs-item <?= ($bFonImg ? 'dark_link color_light--opacity' : '') ?>">
													<?endif;
                                                    ?>

														<span
                                                            class="services-list__item-childs-item-name"><?= $arChild['NAME'] ?></span>
                        ...
                        <div class="services-list__item-text-middle-part">
                            <a class="services-list__item-text-middle-part-link" href="<?= $detailUrl ?>">
                                <span><?= getMessage('MORE_SERVICES') ?></span>
                                <span><i class="svg inline  svg-inline- my_arrow-all__item-arrow" aria-hidden="true"><svg
                                            width="19" height="8" viewBox="0 0 19 8" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
											<path d="M0 3H17V5H0V3Z" fill="url(#paint0_linear_826_2)"></path>
											<g clip-path="url(#clip0_826_2)">
											<path d="M15 1L18 4L15 7" stroke="#12CCF2" stroke-width="2"
                                                  stroke-linecap="round" stroke-linejoin="round"></path>
											</g>
											<defs>
											<linearGradient id="paint0_linear_826_2" x1="-7.91624e-09" y1="4" x2="16"
                                                            y2="4" gradientUnits="userSpaceOnUse">
											<stop stop-color="#0D89E2"></stop>
											<stop offset="1" stop-color="#12CCF2"></stop>
											</linearGradient>
											<clipPath id="clip0_826_2">
											<rect width="5" height="8" fill="white" transform="translate(14)"></rect>
											</clipPath>
											</defs>
											</svg>
											</i> </span>
                        ...
                        <? if ($bShowBottom):?>
                            <div
                                class="services-list__item-text-bottom-part <?= ($bShowPrice ? 'services-list__item-text-bottom-part--has-price' : '') ?>">
                                <? if ($bShowPrice):?>
                                    <div class="services-list__item-price-wrapper">
                                        <div
                                            class="services-list__item-price font_17 <?= ($bFonImg ? 'color_light' : 'color_333') ?>">
                                ...
                                <? if ($arParams['IMAGE_POSITION'] === 'RIGHT' || $arParams['IMAGE_POSITION'] === 'SIDE'):?>
                                    <? if ($bDetailLink):?>
                                        <a class="btn btn-default btn-lg"
                                           href="<?= $detailUrl ?>"><?= Loc::getMessage('SHOW_MORE') ?></a>
                                    <?endif; ?>
                                <?endif; ?>
                            </div>
                        <?endif; ?>
...
<? endif; ?>
    ...
    <div class="bottom_nav mobile_slider <?= ($bHasNav ? '' : ' hidden-nav'); ?>" data-parent=".services-list"
         data-append=".grid-list" <?= ($arParams["IS_AJAX"] ? "style='display: none; '" : ""); ?>>
        <? if ($bHasNav): ?>
            <?= $arResult["NAV_STRING"] ?>
        <? endif; ?>
    </div>
...
<? endif; ?>
    ...
    <div class="bottom_nav_wrapper nav-compact">
        <div class="bottom_nav hide-600" <?= ($arParams['IS_AJAX'] ? "style='display: none; '" : ""); ?>
             data-parent=".services-list" data-append=".grid-list">
